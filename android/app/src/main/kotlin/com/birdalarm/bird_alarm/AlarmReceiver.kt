...
class AlarmReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        // 响铃通知统一由 AlarmSoundService 负责（含「全屏意图」+ 关闭/贪睡动作），
        // 这里不再单独 notify(1001)，也不再手动 startActivity——targetSdk 36 下前台
        // 服务/广播直接拉起 Activity 会被 BAL 拦截。全屏响铃页由通知的全屏意图自动拉起：
        // 锁屏/息屏时自动全屏，亮屏解锁时降级为通知。
    }
